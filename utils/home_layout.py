...
from utils.dish_generator import generate_dishes_and_preferences, visualize_dishes_and_preferences

'''Written using Claude'''

def generate_house_layout_with_dishes():
    """
    Generate a house layout with furniture, people, and dishes with dietary preferences.
    
    Returns:
        Tuple containing:
        - grid: Updated grid with dishes
        - rooms: Room layout information
        - people_positions: List of people positions
        - person_preferences: Dictionary of dietary preferences
        - dish_positions: List of dish positions with types
    """

    def generate_random_layout(grid_size=100, margin=10):
    ...
    grid, rooms = generate_random_layout()
    ...
    # Generate dishes and dietary preferences
    grid_with_dishes, person_preferences, dish_positions = generate_dishes_and_preferences(
        grid, rooms, people_positions, occupied_positions
    )

    # Print summary
    visualize_dishes_and_preferences(grid_with_dishes, person_preferences, dish_positions, people_positions)

    # Save grid to file
    np.savetxt('grid_with_dishes.txt', grid_with_dishes, fmt='%s')
    
    # Visualization setup with dishes
    char_to_int = {
        '.': 0,    # Empty space
        'W': 1,    # Wall
        'L': 2,    # Living Room
    ...
    colors = [
        ...
        'lightgreen',    # Vegan dish
        'lightcoral'    # Non-vegetarian dish
    ]

    # Convert to numeric grid
    numeric_grid = np.zeros_like(grid_with_dishes, dtype=int)
    for char, num in char_to_int.items():
        numeric_grid[grid_with_dishes == char] = num
    ...
    chair_positions = np.where(grid_with_dishes == 'H')
    for y, x in zip(chair_positions[0], chair_positions[1]):
        plt.plot([x-0.5, x+0.5, x+0.5, x-0.5, x-0.5],
                 [y-0.5, y-0.5, y+0.5, y+0.5, y-0.5],
                 'black', linewidth=2)

    # Add person numbers with dietary preference indicators
    for i, pos in enumerate(people_positions):
        person_num = i + 1
        preference = person_preferences.get(person_num, 'unknown')
        pref_symbol = 'V' if preference == 'vegan' else 'M'
        plt.text(pos[1], pos[0], f'{person_num}',
                 color='white', ha='center', va='center',
                 fontsize=7, fontweight='bold')

    # Add dish labels
    for row, col, dish_type in dish_positions:
        dish_symbol = 'V' if dish_type == 'vegan' else 'N'
        plt.text(col, row, dish_symbol,
                 color='black', ha='center', va='center',
    ...
    legend_elements = [Patch(facecolor=color, label=label) 
        ...
        'Vegan Dish', 'Non-Veg Dish'
    ])]
    plt.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(1.2, 1))

    plt.title('House Layout with People, Dietary Preferences, and Dishes\n(V=Vegan, M=Meat/Non-Veg)')
    plt.xlabel('Width')
    plt.ylabel('Height')
    ...
    return grid_with_dishes, rooms, people_positions, person_preferences, dish_positions
